...
class DnsOverHttpsTest(
        ...
        private var mockServerSocket: MockServerSocket? = null
        ...
        fun setUp() {
                ...
                if (socketMode == SocketMode.MOCK) {
                        val mss = MockServerSocket(AutoClock(), NetworkProfile.SLOW_MOBILE)
                        this.mockServerSocket = mss
                        server.serverSocketFactory = mss.asServerSocketFactory()

                        clientBuilder.socketFactory(MockSocketFactory(mss))
                }
        ...
        fun tearDown() {
                val mss = mockServerSocket
                if (mss != null) {
                        println("Events for $socketMode:")
                        mss.sharedEvents.forEach { println("  $it") }
                }
                server.close()
        }
