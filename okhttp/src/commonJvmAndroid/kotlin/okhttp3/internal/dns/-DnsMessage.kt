...
/**
 * An RFC 1035 DNS message, symmetric for requests and responses.
 *
 * These wire-format types live in okhttp core so that both `DnsOverHttps` (which reads them out of
 * HTTP response bodies) and platform resolvers such as Android's `DnsResolver.rawQuery` (which
 * returns them as raw bytes) can share a single decoder.
 *
